...
setGlobals() {
  ...
  if [ $USING_ORG -eq 1 ]; then
    ...
    export CORE_PEER_ADDRESS=localhost:$((7051 + 1000 * $USING_PEER))
  elif [ $USING_ORG -eq 2 ]; then
    export CORE_PEER_LOCALMSPID="Org2MSP"
    export CORE_PEER_TLS_ROOTCERT_FILE=${PWD}/organizations/peerOrganizations/org2.example.com/peers/peer${USING_PEER}.org2.example.com/tls/ca.crt
    export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/org2.example.com/users/Admin@org2.example.com/msp
    export CORE_PEER_ADDRESS=localhost:$((9051 + 1000 * $USING_PEER))
...
setGlobalsCLI() {
  ...
  if [ $USING_ORG -eq 1 ]; then
    export CORE_PEER_ADDRESS=peer${USING_PEER}.org1.example.com:$((7051 + 1000 * $USING_PEER))
  elif [ $USING_ORG -eq 2 ]; then
    export CORE_PEER_ADDRESS=peer${USING_PEER}.org2.example.com:$((9051 + 1000 * $USING_PEER))
  elif [ $USING_ORG -eq 3 ]; then
